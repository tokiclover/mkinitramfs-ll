...
usage() {
  ...
  usages: runned without an argument, build an initramfs for \$(uname -r) with only LUKS support.
...
error() { echo -ne "\033[1;31m* \e[0m$@\n"; }
info() 	{ echo -ne "\033[1;32m* \e[0m$@\n"; }
...
addsplsh() {
	...
	cp -ar "$SPLASH" etc/splash/ || die "splash themes does not exist"
}
...
addmodule() {
	...
	for mod in $@; do
		...
		if [ -z $module ]; then info "$mod does not exist"; return 1
		else 	mkdir -p .${module%/*}
...
if [ -n "$iAUFS" ]; then
	...
	do [ -n "$(echo ${MODULES[2]}|grep $fs)" ] || MODULES[2]=${MODULES[2]},$fs; done
...
modules && unset MODULES

if [ -n "$FSCK" ]; then
	for fs in $(echo $FSCK|tr ',' ' '); do
...
[ -e "$FONT" ] && cp -a "$FONT" etc/ && unset FONT
[ -n "$SPLASH" ] && addsplash && unset SPLASH
