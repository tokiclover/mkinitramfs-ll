#!/bin/zsh
# $Id: mkinitramfs-ll/busybox.zsh,v 0.6.0 2012/05/14 00:42:22 -tclover Exp $
usage() {
  cat <<-EOF
...
if [[ -n ${(k)opts[-n]} ]] || [[ -n ${(k)opts[-minimal]} ]] { make allnoconfig || die
	...
	-e "s|# CONFIG_SWITCH_ROOT is not set|CONFIG_SWITCH_ROOT=y|" -i .config || die "minimal config failed"
