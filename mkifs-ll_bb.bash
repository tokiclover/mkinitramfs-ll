...
usage() {
  ...
  -Y|--key-map <map:kmap>  generate kmap keymap using map as input keymap
...
build_busybox() {
	...
	if [[ -n "${opts[minimal]}" ]]; then
		...
		-e "s|# CONFIG_BLKID is not set|CONFIG_BLKID=y|" \
		-e "s|# CONFIG_BLKID_TYPE is not set|CONFIG_BLKID_TYPE=y|" \
		-e "s|# CONFIG_CAT is not set|CONFIG_CAT=y|" \
		-e "s|# CONFIG_CP is not set|CONFIG_CP=y|" \
		-e "s|# CONFIG_CUT is not set|CONFIG_CUT=y|" \
		-e "s|# CONFIG_FDISK is not set|CONFIG_FDISK=y|" \
		-e "s|# CONFIG_FINDFS is not set|CONFIG_FINDFS=y|" \
		-e "s|# CONFIG_FSCK is not set|CONFIG_FSCK=y|" \
		-e "s|# CONFIG_HEAD is not set|CONFIG_HEAD=y|" \
		-e "s|# CONFIG_GREP is not set|CONFIG_GREP=y|" \
		-e "s|# CONFIG_KBD_MODE is not set|CONFIG_KBD_MODE=y|" \
		-e "s|# CONFIG_INIT is not set|CONFIG_INIT=y|" \
		-e "s|# CONFIG_LOADFONT is not set|CONFIG_LOADFONT=y|" \
		-e "s|# CONFIG_LOADKMAP is not set|CONFIG_LOADKMAP=y|" \
		...
		-e "s|# CONFIG_SETFONT is not set|CONFIG_SETFONT=y|" \
		...
		-e "s|# CONFIG_TTY is not set|CONFIG_TTY=y|" \
