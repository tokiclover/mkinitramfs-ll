@$Header: mkinitramfs-ll/README,v 0.8.2 2012/06/16 12:21:20 -tclover Exp $@
...
on either @{/etc/,}mkinitramfs-ll.conf}@ so one can put many default values there.
...
This is then used to obtain each key file from the specified removable media if any. 
...
To load kernel modules, one could either create needed groups in @/etc/mkinitramfs-ll/module.$group@ 
...
* @sqfsd@     -> modules required for squashfs+aufs2, can be built into the kernel
...
Now one can add scripts to @/etc/local.d@ of the initramfs, or @$gitdir/misc@ if using mkinitramfs-ll 
building script. Each script should have a @.sh@ sufix and a @${ishrl}?-*@ prefix as the follwing:
* runlevel 1: boot and cmdline modules, initialization -> keymap -> font -> 1 -> print boot msg
* runlevel 2?: before... swap: 2s -> resume: 2r -> rootfs
* runlevel 3?: before... decrypt: 3d -> rootfs fsck: 3f -> rootfs mount: 3m ->
* runlevel 4?: before... clean up: 4c -> mount /usr:/var...: 4m -> umount: 4u -> switch root: 4s ->
