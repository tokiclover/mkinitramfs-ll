@Header: mkinitramfs-ll/README.textile,v 0.14.8 2014/12/01 Exp@

bq. "a lightweight, modular and yet powerfull initramfs generating tool
...
An ebuild is available "there":https://github.com/tokiclover/bar-overlay
for Gentoo users.
...
EXAMPLE: @iroot=vg-root:ext4:1 ilvm=yes@ for an ext4 rootfs in LVM (Volume Group
	is named vg), or @iroot=POOL/ROOT izfs=yes@ for a zfs rootfs volume.
...
EXAMPLE: @lvm=pva-sda1:pvb-sdb1,pvc-sdc1@ assigne a single PV to swap group,
...
or @splash=silent,fadein,theme:emergenc,tty:1@ splash kernel cmdline arguments.
...
@usr/lib/mkinitramfs-ll@ if using mkinitramfs-ll.$shell generating script.
...
*Note*: Appending _zram_, or _bcache_ or _zfs_ for that matter, is not required
because @izram@, @izfs@ and @ibcache@ will trigger hook execution respectively.
...
the related mkfs.$fs binaries if the file system string is appended!
...
EXAMPLE: @iroot=root-sda3 video=1280x800-24 imod=hid-multitouch 
...
* @swsusp@: default suspend/resume implementation
