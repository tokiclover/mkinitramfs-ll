@$Header: mkinitramfs-ll/README.textile,v 0.9.8 2012/07/06 00:49:42 -tclover Exp $@
...
a partition number for mdadm softare raid used to: @echo ARRAY <array> <uuid> >>/etc/mdadm.conf@ or
@echo ARRAY <array> devices=/dev/sd?<opt> >> /etc/mdadm.conf@; or else, @<opt>@ is a mdraid software 
(ata)raid metadata format used to scan raid arrays (e.g. asr|..|isw|jmicron|..|dos see @mdraid -l@).
...
Either way, one can omit to embed one and use either: @iraid=[md<n>-]UUID=<uuid>+<opt>@, @md<n>@ is optional.
dmraid software raid can be passed like @iraid=UUID=<uuid>+isw:dos@, ':' as a seperator for metadata format.
