@$Header: mkinitramfs-ll/README.textile,v 0.9.8 2012/07/06 10:14:02 -tclover Exp $@
...
@echo ARRAY <array> devices=/dev/sd?<opt> >> /etc/mdadm.conf@.

Or else, an optional @<format>@ can be used to enable mdraid software (ata)raid e.g. @asr|..|isw|jmicron|..|dos@
see @mdraid -l@. dmraid software raid can be passed like @iraid=<array>+isw:dos@, ':' as a seperator.
