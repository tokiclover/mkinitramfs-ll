@$Header: mkinitramfs-ll/README.textile,v 0.14.0 2014/10/01 10:59:54 -tclover Exp $@
...
EXAMPLE: @iroot=vg-root:ext4:1@ for an ext4 rootfs in LVM (Volume Group is named vg),
	or @iroot=POOL/ROOT:zfs@ for a zfs rootfs (ROOT) in a pool called POOL.
...
KERNEL-COMMAND-LINE-ARG: @isquashd=y[|/squashdir],y[|[n|a]:<dir>]@
DESCRIPTION: squashed directories, see below for more info
EXAMPLE: @isquashd=y,a:lib64@ use default squashed dirs and append @lib64@
...
KERNEL-COMMAND-LINE: imodule
DESCRIPTION: execute colon separated list of named modules or scripts
EXAMPLE: @imodule=zram@

*Note*: Nobody would ever want to append zram module to @imodule@ as zram
module would be executed if @izram@ kernel cmdline argument is present.
...
proper support in squashfs-tools.

SQUASHDIR=<squashed-dir-root> default to @/aufs@
SQUASHD=<colon-sperated-list-of-squashed-dirs> default to @usr:bin:sbin@

Squashed directories argument:

* @isquashd=y[|/squashdir],y[|[n|a]:<dir1>:<dirn>]@
	using default or append directories after a @[n|a]:@
* @isquashd=/squashdir,n:<dir 1>:<dir n>@
	neither SQUASHDIR nor SQUASHD default is used, so everything is required;
* @isquashd=y,y@
	using SQUASHD and SQUASHDIR default values do not require any additional argument;
* @isquashd=y,a:<dir 1>[:<dir n>]@
	same as above, however, @<dir 1>[:<dir n>]@ is  appended to SQUASHD variable.
...
Else, @imodule@ kernel command line could be used to execute a script now and
then e.g. @imodule=zram@ append zram to module execution.
...
@mkinitramfs-ll.$shell -a -fter-g12v -y -M:zfs@
(kernel modules and binaries would be rightly added).

*IMPORTANT*: So either way, one should append @:zfs@ to @iroot@ and/or
to @iswap@ or @iresume@ argument for unencrypted vdev(s); or else, @izfs@ is
required and is enough for encrypted vdev(s) (or both for this later case).

*IMPORTANT*: The @zfs@ suffix should be the *3rd* field for @iswap@ and @iresume@,
so append @:zfs@ to @swap:POOL/VOL:zfs@ which means that no signature is present in
this case; or else @file:POOL/VOL:zfs:sign@ when a signature is present and required.

This is change the default *signature* postion which should be *3rd*. This change
is necessary because a signature is *optional* and *zfs* could be a requirement
(un-encrypted vdev(s)!).
...
Extra actions can be taken care with a *user script* if needed.
...
Supported block device stack is to use bcache near the physical block devices
for the moment untill a sane and simple way is found to be able in insert bcache
in any block stack order.

KERNEL-COMMAND-LIE-ARG: @ibcache=sda:sdb2:sdc2,sdd:sde1@
DESCRIPTION: bcache cache:backing-device block device list, multiple *pairs* are
comma ',' separated.
EXEMPLE: ibcache=sda:sdb1 for a single block device (cache:backing-device)
