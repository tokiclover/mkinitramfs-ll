@$Header: mkinitramfs-ll/README,v 0.7.0 2012/06/05 20:08:16 -tclover Exp $@
...
*Note*:@imount@ can be used to mount /usr:/var ... mount points, each mount point should have a line 
in @/etc/fstab@. Mounting are done with mount options. If those mount device are on unencrypted PV
nothing more is required. However, if those device are LV on LUKS and/or RAID array, one should ensure that
the underlaying PVs are available after swp(), resume() and rootfs() being executed. That mean if the device
are on LVM in the same VG as root, swap or resume, everything should be fine. Just make sure to add the right 
VG in swap or resume VG or RAID array or LUKS PV, if not root group, to make sure that the required volume is 
available even if there are no swap nor resume VG, RAID array or LUKS PV [as if there were one]. 
...
@iraid=md<n>-UUID=<uuid>+<p>@, @[/dev/]md<n>@ is a RAID array, @/dev/@ prefix is optional, 
@UUID=<uuid>@ the UUID of the array and @<p>@ the partition number to scan: 
@echo DEVICE $(ls /dev/sd*<p>) >> /etc/mdadm.conf@. 
if @<p>@ partition number is not present every device will be scanned!
...
One can use squashfs+aufs2 to squash directories like @$PORTDIR:var/lib/layamn@, or system related directories 
like @usr:lib32:lib64:bin:sbin@. Advantages are system speed, responsiveness and very small rootfs footprint.
...
* runlevel 4?: before... 4c -> clean up: 4m -> mount /usr:/var...: 4u -> umount: 4s -> switch root.
