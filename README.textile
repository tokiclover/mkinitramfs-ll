@$Header: mkinitramfs-ll/README,v 0.5.0 2012/05/09 18:10:55 -tclover Exp $@
...
* @ishrl=<n>|:<n>@               -> interrupts init and drops to n level or pass n to real init (see EOF).
...
*LUKS*: @<map-UUID=<uid>[|LABEL=<label>][+[/dev/]sd[a-z0-9]|</path/to/header/file>]@
...
UUID=<uuid> the UUID of the array and <N> the partition number to scan: 
...
* Root--key regular passphrase--and fbsplash: @iroot=sda3 ikroot=pwd video=1280x800-24'@'60 \@
...
*NOTE*: multiple key files (for each group) are supported along with single key file. 
