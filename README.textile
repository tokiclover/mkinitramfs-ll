@$Header: mkinitramfs-ll/README.textile,v 0.9.8 2012/07/05 14:45:21 -tclover Exp $@
...
a partition number for mdadm softare raid used to: @echo DEVICE $(ls /dev/sd*<opt>) >> /etc/mdadm.conf@;
or else, @<opt>@ is a mdraid software (ata)raid metadata format used to scan raid arrays
(e.g. asr|..|isw|jmicron|nvidia|sil|via|dos see @mdraid -l@): @mdraid -ay -f<opt> ...@.
