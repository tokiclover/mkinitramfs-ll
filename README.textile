@$Header: mkinitramfs-ll/README.textile,v 0.9.10 2012/07/06 11:33:36 -tclover Exp $@
...
* @iraid=<array>+UUID=<uuid>[<p>|+<fmt>]@  -> comma separated list of arrays for root, swap and resume.
...
@echo ARRAY <array> <uuid> >> /etc/mdadm.conf@; or else, @iraid=md<n>+<part>@ to enable:
