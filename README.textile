@$Header: mkinitramfs-ll/README.textile,v 0.9.8 2012/07/03 12:36:55 -tclover Exp $@
...
Either, one has an embeded @/etc/mdadm.conf@ in the initramfs, so one could use @iraid=md0+1,md1+2,md2+3@ to 
enable 3 arrays for root, swap and resume when using mdadm software raid. 
