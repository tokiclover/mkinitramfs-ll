@$Header: mkinitramfs-ll/README.textile,v 0.12.0 2012/11/01 19:32:47 -tclover Exp $@
...
*New:* mkinitramfs-ll-0.12.0 allow to be dropped in a shell while being asked
for a block device (infinite loop).
...
*Note*: @:[n|no|N|NO]:@ instead of @[c|chk|y:@ will disable a die after fsck failure.
...
DESCRIPTION: the equivalent of @iroot@ for resume
EXAMPLE: e.g. @iresume=file:<VG-LV>:0x4400@ for a swapfile in a Logical Volume
...
KERNEL-COMMAND-LINE-ARG: @dmesg=emerg|alert|crit|err|..|debug@
DESCRIPTION: optional dmesg log level priority
EXAMPLE: @dmesg=alert@ set dmesg to alert level

init script accept also @single@, @ro@ and rw@ kernel cmdline argument
and will append single runlevel to real init. 

It also support @splash=silent,fadein,theme:emergence console=[/dev/]tty1@
splash kernel cmdline arguments.
...
detached header device|file when using:
...
a block device|partition without the @/dev/@ prefix.
...
* @gpg@: GnuPG encrypted key-file support only gnupg-1.4*,
	...
	in the initramfs root to avoid its creattion on each boot 
...
* @pwd@: regular passphrase: it's not mandatory to append any
	@ikroot[|swap|resume]=pwd@ (fallback keymode)
* @none@ : unencrypted block device which should be explicitely appended 
...
*Warn*: leading '/' slash is only required for @sqfsdir@, else mounting squashed dirs will fail. 
...
EXAMPLE: iroot=sda3 ikroot=pwd video=1280x800-24 imod=drm,drm_kms_helper,ttm,i915 \
	splash=verbose,theme:livecd-20007.0,tty:1

DESCRIPTION: Root--regular key-file--on usb device
EXAMPLE: iroot=sda3 ikroot=reg:sdb1:/path/to/keyfile
...
EXAMPLE: iroot=sda3 iswap=swap[|file]:sda2[:signature] \
...
EXAMPLE: iswap=swap:sda2 iresume=swap:sda3:0x4400 ikswap=pwd ikresume=pwd

DESCRIPTION: Swap file--resuming from hibernation--ldk protected key-file
EXAMPLE: iswap=file:sda3:0x4400 iresume=toi[|CHAR|WORD] ikswap=ldk:sdb1:/path/to/swapkey
...
unencrypted vdev(s) or @:zfs@ to @iswap@ or @iresume@;
