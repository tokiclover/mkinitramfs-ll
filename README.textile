...
EXEMPLE: @iroot=POOL/ROOT izfs=zva-sda:zvb-sdb@
...
So @iroot=POOL/ROOT iswap=swap:POOL/SWAP ikswap=pwd iresume=swap izfs=1,zva-sda:zvba-sdb@
kernel cmdline use a single pool for root, swap and resume. The integer to enable
root on zfs is required for this case because swap and resume group does not mount
any file system. To specify another pool for root, replace the integer by a valid
colon list map-dev.
