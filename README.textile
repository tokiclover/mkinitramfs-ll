@$Header: mkinitramfs-ll/README.textile,v 0.9.6 2012/07/02 15:54:20 -tclover Exp $@
...
Now one can add scripts to @/etc/mkinitramfs-ll@ of the initramfs root, or else in
@$gitdir/usr/etc/mkinitramfs-ll@ if using mkinitramfs-ll.$shell generating script. 
