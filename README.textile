@$Header: mkinitramfs-ll/README,v 0.5.0 2012/05/09 01:54:57 -tclover Exp $@

bq. "initramfs with [RAID][LUKS][LVM][TOI][AUFS+SQUASHFS] support [with a handfull zsh[devel]/bash[master] set of scripts]"
...
* @iroot=<PV|VG-LV>[:c:<fs>:n]@: required root device, recommanded optinal @:c:ext4@ 
...
* @ilvm=<map-PV>,...,<map-PV>@: LVM2 argument @<mapping-PV>@ is collon @:@ separated list 
  of PVs of a VG; and comma @,@ separated list of VG: 1st for root, 2nd for swap and the 
  3rd for resume, hence one can append enough commas to asign a particular group.
...
* @isqfsd=y[|/sqfsdir],y[|[n|a]:<dir1>:<dirn>]@: one can use uv_sqfsd and uv_sqfsdir or append 
  directories after a @[n|a]:@ for sqfsd.
